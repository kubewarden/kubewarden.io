...
Kuberwarden Custom Resources, improvements in Helm Charts, and Status and
...
have a better UX (just define the Custom Resources), can define more than 1
policy-server Deployment, and get to select what ClusterAdmissionPolicies target
which PolicyServers. Let's see a diagram of the new architecture:
...
Each policy server loads different policies, specified in the
ClusterAdmissionPolicies. The new PolicyServer Custom Resource is cluster-wide,
which means that is identifiable by its unique `name`. Here is an example of a
PolicyServer named `tenant-a`:
...
policy server and break havoc in the Cluster. With the new possibility of
several PolicyServers, a Kubernetes Operator can isolate policy evaluations per
tenant/namespace, and run mission critical policies separately, making the whole
...
clusters via Fleet more consistent and streamlined.
...
Now, you can use Kubewarden, with Go, Rust, Swift, Rego policies, as you are
used to.
...
```console
vic@viccuad2 ~/suse/kw/helm-charts/charts (new-architecture=)$ kubectl get policyservers
NAME               AGE
default            1m12s
my-policy-server   29s
```

Meanwhile the new Deployment for the new PolicyServer is not up, the policy will
be marked as `unschedulable`, and move to `pending` once we are waiting for
the PolicyServer to accept connections:
...
the project, and we would like you hear what you would like to see in the
